...
import { PHONE, CardInputRow } from '../utils/frame.jsx'
...
export default function LearningFrame({ topic = 'Sleep', managementAnswers, onComplete, onRestart }) {
  ...
  function handleAnswer(opt) {
    ...
    setTimeout(() => {
      setTyping(false)
      setChatHistory(prev => [...prev, { role: 'petch', text: getPetchResponse(current) }])
            ...
            {current.options.map((opt, i) => (
              <div key={opt}>
                <button
                  ...
                  style={{ height: 40 }}
                >
                  <div className="shrink-0 flex items-center justify-center rounded-[20px]" style={{ width: 40, height: 40, background: '#33CCFF' }}>
                    <span style={{ ...DIN, fontWeight: 400, fontSize: 18, color: 'white', letterSpacing: '-0.25px' }}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                  </div>
                  <span className="flex-1 text-left text-white" style={{ ...DIN, fontWeight: 400, fontSize: 18, letterSpacing: '-0.25px' }}>
                    {opt}
                  </span>
                </button>
                <div style={{ height: 1, background: 'rgba(255,255,255,0.25)', margin: '6px 0' }} />
